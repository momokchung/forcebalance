...
gen_opts_types = {
    'strings' : {"gmxpath"      : (which('mdrun'), 60, 'Path for GROMACS executables (if not the default)', 'All targets that use GROMACS', ['GMX']),
                 ...
                 "tinker9path"  : (None, 60, 'Path for TINKER9 executables', 'All targets that use TINKER9', ['TINKER']),
...
tgt_opts_types = {
    ...
    'floats'  : {"weight"       : (1.0, 150, 'Weight of the target (determines its importance vs. other targets)', 'All targets (important)'),
                 ...
                 "potential_denom"   : (1.0, 0, 'Potential normalization (Kcal/mol/charge) ; set to 0 if a zero weight is desired', 'Monomer property targets', 'monomer'),
